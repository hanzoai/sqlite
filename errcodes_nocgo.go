...
// backend (same build condition as driver_nocgo.go / hooks_nocgo.go, including
// the sqlite_purego opt-out that forces modernc under cgo). The values come
// from modernc's generated lib package and are the same SQLite extended result
// codes mattn exposes under the CGO build.
var (
