...
import (
	...
	"github.com/hanzoai/sqlite/internal/engine/vtab"
)
...
// Connector returns a driver.Connector for dsn backed by the registered driver.
// sql.OpenDB(Connector(dsn)) is equivalent to sql.Open(driverName, dsn), but lets
// a caller wrap the returned connector in an io.Closer whose Close runs when the
// sql.DB is closed. The pure-Go SQLCipher codec path uses that to unregister the
// per-database codec VFS exactly when its *sql.DB closes.
func Connector(dsn string) driver.Connector { return &connector{dsn: dsn} }

type connector struct{ dsn string }

func (c *connector) Connect(_ context.Context) (driver.Conn, error) { return d.Open(c.dsn) }
