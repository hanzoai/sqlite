...
func applyQueryParams(c *conn, query string) error {
	...
	// _reserve_bytes=N sets SQLITE_FCNTL_RESERVE_BYTES before ANY writing pragma
	// (journal_mode=WAL touches page 1). The pure-Go SQLCipher codec VFS needs an
	// 80-byte per-page reserve for the IV+HMAC trailer; on a new database this must
	// be set before page 1 is written.
	if v := q.Get("_reserve_bytes"); v != "" {
		n, err := strconv.Atoi(v)
		if err != nil || n < 0 || n > 255 {
			return fmt.Errorf("invalid _reserve_bytes %q", v)
		}
		if err := c.fileControlReserveBytes(n); err != nil {
			return err
		}
	}
